        ...
        async.each(root, function(child, cb){
            ...
            async.each(to, function(receipient, cb){
                ...
                if(sceneIndex > -1){
                    ...
                    var tables = {};
                    tables[child._source._table] = [child._source.hashid];
                    data.scenes.push({duration: 0, start: 0, chars: chars, id: data.scenes.length, date: new Date(child._source[from_field]).getTime() || 0, 'tables': tables, 'hashids': [child._source.hashid]})
                }
                cb();
            });
        }, function(err){
