        ...
        async.each(root, function(child, cb){
            ...
            async.each(people, function(receipient, cb){
                ...
                if(toIndex > -1){ //Receipient has entry
                    ...
                    data.names.push({'id': data.names.length, 'name' : receipient, 'group':0, 'sent' : 0, 'received' : 1, 'hashids' : [child._source.hashid], 'tables': tables});
                } 
                cb();
            }, function(err){
                cb();
        ...
        async.each(root, function(child, cb){
            ...
            async.each(to, function(receipient, cb){
                ...
                if(sceneIndex > -1){
                    ...
                    data.scenes.push({duration: 10, start: 0, chars: chars, id: data.scenes.length, date: new Date(child._source[from_field]).getTime() || 0})
                }
                cb();
            });
        }, function(err){
