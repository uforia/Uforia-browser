...
function getUser(userId, cb) {
  c.elasticsearch.search({
    ...
    _source: ["id", "firstName", "lastName", "email", "isDeleted", "role", "preferences"],
  }, cb);
}
...
/**
 * Gets the active case for the currently logged in user.
 */
router.get('/get_active_case', function (req, res) {
  var userId = req.query.userId;
  
  getUser(userId, function (error, response) {
    var activeCase = response.hits.hits[0]._source.preferences.cases.activeCase;

    res.send({ error: error, response: activeCase });
  });
});
